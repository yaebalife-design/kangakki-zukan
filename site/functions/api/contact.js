/**
 * お問い合わせの受け口（Cloudflare Pages Functions）— 管楽器図鑑（スキマー図鑑と同方式）
 *
...
async function ipHash(ip) {
  const buf = await crypto.subtle.digest("SHA-256", new TextEncoder().encode("kangakki:" + ip));
  const a = new Uint8Array(buf);
  let s = "";
